...
document.getElementById("typed_text").addEventListener("input", (e) => {
    ...
    if (highlightEnabled) {
        ...
        const currentWordIndex = typedWords.length; // Get the index of the current word being typed

        // Highlight the current word
        const highlightedText = currentWords.map((word, index) => {
            return index === currentWordIndex - 1 ? `<span class="highlight">${word}</span>` : word;
        }).join(" ");

        document.getElementById("test_text").innerHTML = `<div class="word-set">${highlightedText}</div>`; // Update the displayed text
    } else {
        // If highlighting is not enabled, display the words normally
    ...
    if (typedText) {
        ...
        typedChunks[chunkIndex] = typedText; // Store typed text for the current chunk
    }
});
